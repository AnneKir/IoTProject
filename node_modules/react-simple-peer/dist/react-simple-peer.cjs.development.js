...
var freeice = _interopDefault(require('freeice'));
var SimplePeer = _interopDefault(require('simple-peer'));
var uid = _interopDefault(require('uid-safe'));
var typedStructures = require('typed-structures');
var React = _interopDefault(require('react'));
var filterReactProps = _interopDefault(require('filter-react-props'));
var wasmFlate = require('wasm-flate');
...
var processOfferRequest = function processOfferRequest(request) {
  try {
    ...
    if (ReactSimplePeerStatusState.value !== exports.Status.IDLE) {
      state.joinRequests.stack(request);
      ReactSimplePeerState.next(state);
      return Promise.resolve();
    }

    ReactSimplePeerStatusState.next(exports.Status.EMITTING_OFFER);
    var config = {
      initiator: true,
      trickle: false,
      config: {
...
var send = function send(pc, command, data) {
  try {
    try {
      pc.send(wasmFlate.deflate_encode_raw(new Uint8Array(Buffer.from("" + ReactSimplePeerState.value.id + command + data))));
...
function handlePeerConnection(pc, openConnectionAsInitiator) {
  ...
  pc.on('data', function (data) {
    if (!pc.initiator && ReactSimplePeerState.value.connections.size === 0) {
      ReactSimplePeerStatusState.next(exports.Status.CONNECTING_TO_EXISTING_PEERS);
    }

    parse(new TextDecoder('utf-8').decode(wasmFlate.deflate_decode_raw(data)), ReactSimplePeerState.value);
...
var processJoinResponse = function processJoinResponse(offer, id, room) {
  try {
    ...
    if (state.connections.size === 0) {
      ReactSimplePeerStatusState.next(exports.Status.RECEIVING_RESPONSE_ACCESS);
    }
    ...
    return Promise.resolve(getSignalData(state.peerConnection)).then(function (signalData) {
      if (state.connections.size === 0) {
        ReactSimplePeerStatusState.next(exports.Status.JOINING_SESSION);
      }
...
function processClientOffer(data, id, roomCreator, emitterPeerId) {
  ...
  ReactSimplePeerStatusState.next(exports.Status.IDLE);
  ReactSimplePeerState.next(state);
}
...
/*#__PURE__*/
new rxjs.BehaviorSubject(exports.Status.IDLE);
var ReactSimplePeerState =
/*#__PURE__*/
new rxjs.BehaviorSubject(initState);
...
ReactSimplePeerStatusState.subscribe(function (status) {
  try {
    if (status === exports.Status.IDLE && ReactSimplePeerState.value.joinRequests.length()) {
      var state = ReactSimplePeerState.value;
...
exports.ClientOffer = ClientOffer;
exports.JoinRequest = JoinRequest;
exports.JoinRoomButton = JoinRoomButton;
exports.PeerVideo = PeerVideo;
exports.Peers = Peers;
exports.ReactSimplePeerState = ReactSimplePeerState;
exports.ReactSimplePeerStatusState = ReactSimplePeerStatusState;
exports.Setup = Setup;
exports.State = State;
exports.getId = getId;
exports.setModel = setModel;
exports.setRoom = setRoom;
//# sourceMappingURL=react-simple-peer.cjs.development.js.map
